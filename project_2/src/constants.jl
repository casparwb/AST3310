...
const Ps = P_func(ρs, Ts)      # solar surface pressure
const Cp = 5/2*k_B/(μ*m_u)     # heat capacity at constant pressure
κ_table = read_opacity()       # get opacity arrays from file

solar_inits = [Rs, ρs, Ts]     # solar initial parameter values
best_inits = [1.5*Rs, 1.2*ρs, 0.9*Ts, 0.8*Ps] # initial values for the best model
