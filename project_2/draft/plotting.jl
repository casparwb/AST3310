...
function circle(r)
    """ Return x, and y-coordinates for a circle
    with radius r and centre in (0 ,0) """

    θ = collect(LinRange(0, 2*π, 100))
...
function plot_nablas(inits=nothing, modelname=nothing)
    ...
    for i = 1:length(r)
        ...
        ∇_stable[i] = get_∇_stable(κ, L, P, T, m)
...
function plot_flux(inits=nothing, modelname=nothing)
    ...
    for i = 1:length(R)
        ...
        ∇_stable = get_∇_stable(κ, L[i], P[i], T[i], m[i])
