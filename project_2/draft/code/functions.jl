...
""" some simple parameter functions """
scale_height(T, g) = k_B*T/(μ*m_u*g)
geometric_factor(lm) = 4.0/lm                  # gas parcel geometric factor S/(Q*lm*d)
...
function get_∇(u, m, ρ, ∇_ad)
    ...
    if ∇_stable < ∇_ad
        ...
        # unstable
        ∇T = ∇T_con(u, m, κ, ρ, Hp, g, ∇_ad, ∇_stable)
        stability = 0
    end
...
function get_opacity(T_in, ρ_in, verbose=false, round=false)
    ...
    if any(T_eq) && any(R_eq) # if exact value is present
        ...
        if logT_in < logT[1] || logT_in > logT[end] || logR_in < logR[1] || logR_in > logR[end]
            if verbose println("Input parameter out of bounds, extrapolating opacity") end
